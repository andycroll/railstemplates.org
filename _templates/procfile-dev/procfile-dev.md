...
- Writes `Procfile.dev` with:
  - **`web:`** — always included, prefixed with `RUBY_DEBUG_OPEN=true` so [`debug`](https://github.com/ruby/debug) opens a remote console you can attach to.
...
- An existing `web:` line is preserved verbatim, except `RUBY_DEBUG_OPEN=true` is prepended if missing.
